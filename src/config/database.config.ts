...
export default registerAs('database', () => ({
  ...
  extra: {
    ...
    idleTimeoutMillis: parseInt(process.env.DB_POOL_IDLE_TIMEOUT || '30000', 10), // How long a client can remain idle before being closed
    connectionTimeoutMillis: parseInt(process.env.DB_CONNECTION_TIMEOUT || '10000', 10), // How long to wait for a connection
  },

  // TypeORM specific settings
  synchronize: process.env.NODE_ENV === 'development', // Auto-create database schema (only in development)
  logging: process.env.NODE_ENV === 'development' ? ['query', 'error', 'warn'] : ['error'], // Log queries in development

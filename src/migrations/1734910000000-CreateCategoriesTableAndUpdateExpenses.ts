...
export class CreateCategoriesTableAndUpdateExpenses1734910000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createIndex('categories', new TableIndex({ name: 'IDX_categories_name', columnNames: ['name'], isUnique: true }));
    ...
    if (hasExpensesTable) {
      await queryRunner.addColumn('expenses', new TableColumn({ name: 'category_id', type: 'uuid', isNullable: true }));
      await queryRunner.changeColumn('expenses', 'category_id', new TableColumn({ name: 'category_id', type: 'uuid', isNullable: false }));
      await queryRunner.createIndex('expenses', new TableIndex({ name: 'IDX_expenses_category_id', columnNames: ['category_id'] }));
      await queryRunner.createForeignKey('expenses', new TableForeignKey({
        columnNames: ['category_id'], referencedColumnNames: ['id'], referencedTableName: 'categories', onDelete: 'RESTRICT', name: 'FK_expenses_category_id'
      }));
    }
  }
